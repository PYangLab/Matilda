...
def main_task(rna,adt,atac,cty,batch_size=64,z_dim=100,hidden_rna=185,hidden_adt=30,hidden_atac=185,seed=1,
             classification=False,query=False,fs=False,
             fs_method="IntegratedGradient",dim_reduce=False,simulation=False,simulation_ct="CD16 Mono",simulation_num=100):
    ...
    if adt != "NULL" and atac != "NULL":
        ...
        dl = DataLoader(transformed_dataset, batch_size=batch_size, shuffle=False, num_workers=0, drop_last=False)
    ...
    #transform_real_label = real_label(ref_cty, classify_dim)
    ...
    if (simulation == True) and (simulation_ct!="-1"):
        print("simulate celltype:", simulation_ct) #transform_real_label[simulation_ct]
        checkpoint_tar = os.path.join(model_save_path, 'model_best.pth.tar')
        if os.path.exists(checkpoint_tar):
            checkpoint = torch.load(checkpoint_tar)
            model.load_state_dict(checkpoint['state_dict'], strict=True)
        ...
        sim_cty_index = transform_real_label.index(simulation_ct)
        index = (label == sim_cty_index).nonzero(as_tuple=True)[0]
        aug_fold = int(simulation_num/int(index.size(0)))    
        remaining_cell = int(simulation_num - aug_fold*int(index.size(0)))

        index = (label == sim_cty_index).nonzero(as_tuple=True)[0]
        anchor_data = data[index.tolist(),:]
        anchor_label = label[index.tolist()]
        anchor_transform_dataset = MyDataset(anchor_data, anchor_label)
        anchor_dl = DataLoader(anchor_transform_dataset, batch_size=batch_size,shuffle=True, num_workers=0,drop_last=False)
        new_data = []
        new_label = []
...
def run_matilda_task():
    arguments = matilda_args_main_task()
    main_task(**arguments)
