...
def main_train(rna,adt,atac,cty,batch_size=64,epochs=30,lr=0.02,z_dim=100,hidden_rna=185,hidden_adt=30,hidden_atac=185,seed=1,augmentation=True):
    ...
    if adt != "NULL" and atac != "NULL":
        mode = "TEAseq"
        train_rna_data_path = rna
        train_adt_data_path = adt
        train_atac_data_path = atac
        train_label_path = cty
        ...
        train_dl = DataLoader(train_transformed_dataset, batch_size=batch_size, shuffle=True, num_workers=0, drop_last=False)
    ...
    #######build model#########
    if mode == "CITEseq":
        model = CiteAutoencoder_CITEseq(nfeatures_rna, nfeatures_adt, hidden_rna, hidden_adt, z_dim, classify_dim)
    elif mode == "SHAREseq":
        model = CiteAutoencoder_SHAREseq(nfeatures_rna, nfeatures_atac, hidden_rna, hidden_atac, z_dim, classify_dim)
    elif mode == "TEAseq":
        model = CiteAutoencoder_TEAseq(nfeatures_rna, nfeatures_adt, nfeatures_atac, hidden_rna, hidden_adt, hidden_atac, z_dim, classify_dim)

    #model = nn.DataParallel(model).to(device) #multi gpu
    model = model.to(device) #one gpu
    ########train model#########
    model, acc1, num1, train_num = train_model(model, train_dl, test_dl, lr=lr, epochs=epochs, classify_dim = classify_dim, best_top1_acc=0, save_path=model_save_path,feature_num=feature_num)
    ...
    if augmentation == True:
        ...
        for anchor in minor_anchor_cts:
            ...
            anchor_dl = DataLoader(anchor_transfomr_dataset, batch_size=batch_size,shuffle=True, num_workers=0,drop_last=False)
    ...
    ############## train model ###########
    model,acc2,num1,train_num = train_model(model, train_dl, test_dl, lr=lr, epochs=int(epochs/2),classify_dim=classify_dim,best_top1_acc=0, save_path=model_save_path,feature_num=feature_num)
    checkpoint_tar = os.path.join(model_save_path, 'model_best.pth.tar')
    if os.path.exists(checkpoint_tar):
        checkpoint = torch.load(checkpoint_tar)
        model.load_state_dict(checkpoint['state_dict'], strict=True)
        print("load successfully")
    model,acc2,num1,train_num = train_model(model, train_dl, test_dl, lr=lr/10, epochs=int(epochs/2),classify_dim=classify_dim,best_top1_acc=0, save_path=model_save_path,feature_num=feature_num)


    transform_real_label = real_label(cty, classify_dim)
    pd.DataFrame(transform_real_label, columns=['CellType']).to_csv("real_cty.csv", index=False, header=False)
    return "Training Complete"

def run_matilda_train():
    arguments = matilda_args_main_train()
